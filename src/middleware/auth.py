...
async def _resolve_user(db: AsyncSession, user_id: uuid.UUID, organization_id: uuid.UUID) -> User:
...
async def _resolve_client(db: AsyncSession, client_id: uuid.UUID, organization_id: uuid.UUID) -> Client:
    result = await db.execute(
        select(Client)
...
async def get_current_principal(
    ...
    return Principal(id=user.id, organization_id=user.organization_id, organization=user.organization)
...
async def get_api_key_principal(
    ...
    payload = _decode_token(credentials.credentials)
    if payload.get("type") != "api_key":
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="API key authentication required")
