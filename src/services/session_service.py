...
async def create_session(
    ...
    event_data = [
        compute_raw_event_hash(sess.id, i, ev.payload)
        for i, ev in enumerate(data.events)
    ]

    session_tree = build_tree([eh for _, eh in event_data])
    sess.session_root = session_tree.root
    ...
    db.add_all(
        [
            Event(
                session_id=sess.id,
                sequence_no=i,
                data_hash=event_data[i][0],
                event_hash=event_data[i][1],
            )
...
async def add_events(
    ...
    for i, ev in enumerate(data.events):
        seq_no = base_seq + i
        data_hash, event_hash = compute_raw_event_hash(sess.id, seq_no, ev.payload)
        event = Event(
            session_id=sess.id,
            sequence_no=seq_no,
            data_hash=data_hash,
...
def build_all_proofs(
    session_id: uuid.UUID, all_events: list[Event], tree
) -> list[dict]:
    proofs = []
...
async def list_sessions(
    ...
    total = (
        await db.execute(select(func.count()).select_from(Session).where(cond))
    ).scalar_one()
...
async def get_event_proof(
    ...
    computed_data_hash, computed_event_hash = compute_raw_event_hash(
        session_id, sequence_no, payload
    )
    ...
    if event is None or event.event_hash != computed_event_hash:
        return None
    ...
    leaf_index = next(
        i for i, e in enumerate(all_events) if e.sequence_no == sequence_no
    )
